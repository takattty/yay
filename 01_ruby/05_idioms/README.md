...
`%w` と `%i` には、それぞれ対になる `%W` と `%I` が存在します。これらの違いは、**式展開とバックスラッシュ記法が有効かどうか**です。
...
動的な初期化が必要だけれども、**2回目以降はその結果をキャッシュしたい**ときなどによく目にする表現です。
...
[1, 2, 3].map(&:even?) # => [false, true, false] 今回だとeven?メソッドを呼び出している。かつレシーバーの省略もしている
```
